...
if(isset($_POST["action"])&&($_POST["action"]=="join")){
	...
	if ($RecFindUser->num_rows>0){
		header("Location: http://localhost/member/member_join.php?errMsg=1&username={$_POST["m_username"]}");
	}else{
	//若沒有執行新增的動作	
		...
		$stmt->close();
		$db_link->close();
		header("Location: http://localhost/member/member_join.php?loginStats=1");
	}
}
?>
<html>
